...
static void fill_symtab(Context &ctx) {
  ctx.symtab.add(ctx, "__dyld_private", N_SECT, false, 8, 0x0);
  ctx.symtab.add(ctx, "__mh_execute_header", N_SECT, true, 1, 0x10);
  ctx.symtab.add(ctx, "_hello", N_SECT, true, 1, 0x0);
  ctx.symtab.add(ctx, "_main", N_SECT, true, 1, 0x0);
  ctx.symtab.add(ctx, "_printf", N_UNDF, true, 0, 0x100);
  ctx.symtab.add(ctx, "dyld_stub_binder", N_UNDF, true, 0, 0x100);
}
